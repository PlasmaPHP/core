...
class TypeExtensionsManager {
     ...
     * @return TypeExtensionsManager
     * @throws Exception  Thrown if the name does not exist.
     */
    static function getManager(?string $name = null): TypeExtensionsManager {
        if($name === null) {
     ...
     * @param string                      $name
     * @param TypeExtensionsManager|null  $manager  If `null` is passed, one will be created.
     * @return void
     * @throws Exception  Thrown if the name is already in use.
     */
    static function registerManager(string $name, ?TypeExtensionsManager $manager = null): void {
        if(isset(static::$instances[$name])) {
     ...
     * @param TypeExtensionInterface  $type
     * @return void
     * @throws Exception  Thrown if the type identifier is already in use.
     */
    function registerType($typeIdentifier, TypeExtensionInterface $type): void {
        if(isset($this->regularTypes[$typeIdentifier]) || isset($this->classTypes[$typeIdentifier])) {
            throw new Exception('Type identifier is already in use');
        }
     ...
     * @throws Exception  Thrown if the type identifier is already in use.
     */
    function registerDatabaseType($typeIdentifier, TypeExtensionInterface $type): void {
        if(isset($this->dbTypes[$typeIdentifier])) {
            throw new Exception('Database Type identifier is already in use');
        }
     ...
     * @throws Exception  Thrown if unable to encode the value.
     */
    function encodeType($value, ColumnDefinitionInterface $column): TypeExtensionResultInterface {
        $type = \gettype($value);
        if($type === 'double') {
            $type = 'float';
        }
        ...
        throw new Exception('Unable to encode given value');
    }
     ...
     * @param mixed       $value
     * @return TypeExtensionResultInterface
     * @throws Exception  Thrown if unable to decode the value.
     */
    function decodeType($type, $value): TypeExtensionResultInterface {
        ...
        throw new Exception('Unable to decode given value');
    }
}
